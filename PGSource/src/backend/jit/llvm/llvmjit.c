 ...
 */
LLVMModuleRef
llvm_mutable_module(LLVMJitContext *context, const char *ModuleID)
{
	llvm_assert_in_fatal_section();
	...
	if (!context->module)
	{
		...
		context->module = LLVMModuleCreateWithName(ModuleID);
...
llvm_optimize_module_select(LLVMModuleRef module, bool flag_opt3, bool flag_inline)
{
	...
	printf("[MOD OPTIMIZING SELECTED] %d\n", compile_optlevel);
...
llvm_compile_module(LLVMJitContext *context)
{
	...
	if(context->module_inline)
	{
		printf("[INLINE-MODULE_INLINE]\n");
		llvm_inline(context->module_inline);
	}
	if(context->module_opt) 
	{
		printf("[INLINE-MODULE_OPT]\n");
