...
TransactionPoolInit(void)
{
	...
	{ 	
		...
		for(int i=0; i < TXN_SIZE; i++)
		{
			...
			transactionPool->transactions[i]->workingset.dataId			=	0;
...
NewOrderSQLData 
parse_data(char *data_str) 
{
	// Create a buffer to store the cleaned string
    char buffer[MAX_LINE_LENGTH];
    int j = 0;
    for (int i = 0; data_str[i] != '\0'; i++) 
	{
        if (data_str[i] != '(' && data_str[i] != ')') {
            buffer[j++] = data_str[i];
        }
    }
    buffer[j] = '\0'; // Null-terminate the buffer
	
    NewOrderSQLData data;
    sscanf(buffer, "%lu, %lu, %lu, %lu, %lu, %lu",
           &data.typeid, &data.tableid, &data.wid, &data.did, &data.cid, &data.iid);
    data.hashValue = pc3_hash_value(data.tableid, data.wid, data.did, data.cid, data.iid); 
    return data;
}

void 
parse_csv_line(char *line, PC3HashKey *entry, bool printLog) 
{
    char *start_input = strchr(line, '"');

    // Find the second quote which indicates the end of the input part
    char *end_input = strchr(start_input + 1, '"');

    // Extract the input part
    *end_input = '\0';
    char *input_str = start_input + 1;

    // Find the third quote which indicates the start of the output part
    char *start_output = strchr(end_input + 1, '"');
    // Find the fourth quote which indicates the end of the output part
    char *end_output = strchr(start_output + 1, '"');

    *end_output = '\0';
    char *output_str = start_output + 1;
    ...
    while (token != NULL && idx < MAX_ENTRIES) 
	{
        entry->workingset[idx++] = parse_data(token+1);
		entry->dataId += entry->workingset[idx-1].hashValue;
        token = strtok(NULL, ")");
    }
    ...
    entry->outputSize = idx - entry->inputSize;

    // Fill remaining entries with zeroed data
    for (int i = idx; i < MAX_ENTRIES; i++) 
	{
        memset(&entry->workingset[i], 0, sizeof(NewOrderSQLData));
    } 
}

void 
process_csv_file(const char *filename, bool printLog) 
{
	...
	{
		PC3HashKey temp_hash;

        line[strcspn(line, "\n")] = 0;  	// Remove newline character

        parse_csv_line(line, &temp_hash, printLog);

		if(temp_hash.workingset[0].typeid == 0 && temp_hash.workingset[0].tableid == 0)
			continue;

		index = temp_hash.dataId % PCCC_HASH_SIZE;
		...
		while(predictionData->pc3HashTable[index].dataId != 0)
		{
			...
			{
				elog(ERROR, "PC3 Cache Table Full!");
				return;
			}
		}

		predictionData->pc3HashTable[index] = temp_hash;

        if(printLog && predictionData->pc3HashTable[index].inputSize==kValue)
        {
			printf("Index: %zu, Input size: %zu, Output size: %zu, DataId: %ld\n", index, predictionData->pc3HashTable[index].inputSize, predictionData->pc3HashTable[index].outputSize, predictionData->pc3HashTable[index].dataId);
	        for (int i = 0; i < MAX_ENTRIES; i++) 
			{
	            printf("Entry %d: (%lu, %lu, %lu, %lu, %lu, %lu)  Hash: %lu\n",
	                   i, predictionData->pc3HashTable[index].workingset[i].typeid, predictionData->pc3HashTable[index].workingset[i].tableid, 
	                   predictionData->pc3HashTable[index].workingset[i].wid, predictionData->pc3HashTable[index].workingset[i].did, 
	                   predictionData->pc3HashTable[index].workingset[i].cid, predictionData->pc3HashTable[index].workingset[i].iid, 
					   predictionData->pc3HashTable[index].workingset[i].hashValue);
			}
		} 
    }
...
InitPC3Cache(void)
{
    ...
    predictionData = (PC3Cache*)ShmemInitStruct("PC3Cache", sizeof(PC3Cache), &found);
 
    if (!found)
    {
		for(int i=0; i<PCCC_HASH_SIZE; i++)
		{	
			predictionData->pc3HashTable[i].dataId 		= 	0;
			predictionData->pc3HashTable[i].inputSize	=	0;
			predictionData->pc3HashTable[i].outputSize	=	0;
			for(int j=0; j<MAX_ENTRIES; j++)
			{
				predictionData->pc3HashTable[i].workingset[j].hashValue	=	0;
				predictionData->pc3HashTable[i].workingset[j].wid	=	0;
				predictionData->pc3HashTable[i].workingset[j].did	=	0;
				predictionData->pc3HashTable[i].workingset[j].cid	=	0;
				predictionData->pc3HashTable[i].workingset[j].iid	=	0;
				predictionData->pc3HashTable[i].workingset[j].typeid	=	0;
				predictionData->pc3HashTable[i].workingset[j].tableid	=	0;
			}
		}

        process_csv_file("/opt/60_output.csv", true);

        /* predictionData->hashTable = ShmemInitHash("PC3CacheHash",
                                                         PCCC_HASH_SIZE, 
                                                         PCCC_HASH_SIZE, 
                                                         &ctl,
