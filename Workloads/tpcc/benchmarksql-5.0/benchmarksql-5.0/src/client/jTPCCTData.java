...
public class jTPCCTData
{
    ...
    public void generateNewOrder(Logger log, jTPCCRandom rnd, long due, int ooid)
    {
	int     o_ol_cnt;
	int     i = 0;
	...
	newOrder.w_id   = 1;	//terminalWarehouse;    // 2.4.1.1
	newOrder.d_id   = jTPCC.csv.oorder.oo_d_id.get(ooid);   // 2.4.1.2
	newOrder.c_id   = jTPCC.csv.oorder.oo_c_id.get(ooid);
	o_ol_cnt        = jTPCC.csv.oorder.oo_ol_cnt.get(ooid);   // 2.4.1.3

	newOrder.ol_data = jTPCC.csv.orderline.ol_data.get(jTPCC.olstr);

	//System.out.println(jTPCC.olstr);
	
	while (i < o_ol_cnt)                    // 2.4.1.5
	{

	    newOrder.ol_i_id[i]         = jTPCC.csv.orderline.ol_i_id.get(ooid);
	   // if (rnd.nextInt(1, 100) <= 99)
		//newOrder.ol_supply_w_id[i] = terminalWarehouse;
	   // else

	   	// System.out.println("======================");
		// System.out.print(ooid);
		// System.out.print("   ");
		// System.out.print(newOrder.c_id);
		// System.out.print("    ");
		// System.out.println(newOrder.ol_i_id[i]);

		newOrder.ol_supply_w_id[i] = 1;	//rnd.nextInt(1, numWarehouses);
	    newOrder.ol_quantity[i] = jTPCC.csv.orderline.ol_quantity.get(i+jTPCC.olstr);
	    i++;
	}

	// for(int i=0;i<jTPCC.csv.numol;i++)
	// {
...
//	ResultSet               rs1;
//	stmt1 = db.stmtNewOrderSelectoid;
//	int fid = jTPCC.csv.oorder.oo_d_id.get(newOrder.d_id); //fid 假的区域id
//	stmt1.setString(1, jTPCC.csv.dmap2.get(fid));
//	rs1 = stmt1.executeQuery();
//	if (!rs1.next())
//	{
//	rs1.close();
//	throw new SQLException("District for" +
//		" d_name=" + jTPCC.csv.dmap2.get(fid) +
//		" not found");
//	}
//	int tid = rs1.getInt("d_id");
//	newOrder.d_id   = tid;
//***********************************
	...
	try {
		...
		// insertOrderLineBatch.setString(9, newOrder.ol_data);
	...
	*/
    ...
    public void generatePayment(Logger log, jTPCCRandom rnd, long due,int ooid)
    {
	transType       = TT_PAYMENT;
	transDue        = due;
	transStart      = 0;
	...
	payment.w_id    = 1;//terminalWarehouse;    // 2.5.1.1
	payment.d_id    = ooid;   // 2.5.1.2
	payment.c_w_id  = payment.w_id;
	payment.c_d_id  = payment.d_id;
	//if (rnd.nextInt(1, 100) > 85)
	//{
	...
	//{
	payment.c_last = null;
	payment.c_id = jTPCC.csv.oorder.oo_c_id.get(ooid);
	//}
	//else
	//{
	 //   payment.c_last = null;
	...
	// 2.5.1.3
	double oamount=0;
	for(int j=0;j<jTPCC.csv.oorder.oo_ol_cnt.get(ooid);j++){
		oamount = oamount+jTPCC.csv.orderline.ol_amount.get(j+jTPCC.olstr);
	}
	payment.h_amount = oamount;
    }
...
//*******************************        
	...
	stmt1 = db.stmtNewOrderSelectoid;
	int fid = jTPCC.csv.oorder.oo_d_id.get(payment.d_id); //fid 假的区域id
	stmt1.setString(1, jTPCC.csv.dmap2.get(fid));
	rs1 = stmt1.executeQuery(); 
	if (!rs1.next())
	{
...
//***********************************    
	...
	try
	{
		...
		stmt.setInt(4, payment.c_d_id); //payment.c_d_id
		stmt.setInt(5, payment.c_id);//payment.c_id
		stmt.executeUpdate();
	    }
	    else
	    {
		...
		stmt.setInt(5, payment.c_d_id);//payment.c_d_id
		stmt.setInt(6, payment.c_id);//payment.c_id
		stmt.executeUpdate();
	    }
...
log.trace("w_zip=" + payment.w_zip + " d_zip=" + payment.d_zip);
	...
	/*    stmt = db.stmtOrderStatusSelectCustomer;
	    ...
	    stmt.setInt(1, orderStatus.w_id);
	    stmt.setInt(2, 1);//orderStatus.d_id
	    stmt.setInt(3, 296);//orderStatus.c_id
	    stmt.setInt(4, orderStatus.w_id);
	    stmt.setInt(5, 1);//orderStatus.d_id
	    stmt.setInt(6, 296);//orderStatus.c_id
	    rs = stmt.executeQuery();
	    if (!rs.next())
	    {
	...
	/*	stmt1 = db.stmtDeliveryBGUpdateOrderLine;
		...
		stmt1.setInt(2, deliveryBG.w_id);
		stmt1.setInt(3, d_id);  //d_id
		stmt1.setInt(4, c_id);//c_id
		stmt1.executeUpdate();
