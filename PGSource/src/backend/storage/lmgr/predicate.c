	...
	if (!sxid)
	{
		...
		if (conflictCommitSeqNo != 0)
		{
			if (conflictCommitSeqNo != InvalidSerCommitSeqNo
				&& (!SxactIsReadOnly(MySerializableXact) && !pc3 
					|| conflictCommitSeqNo
			...
			if (SxactHasSummaryConflictIn(MySerializableXact) && !pc3 
										...
										SERIALIZABLEXACT *writer)
{
	...
	if (failure)
	{
		 ...
		 */
		if (MySerializableXact == writer && !pc3)
		{
			LWLockRelease(SerializableXactHashLock);
			ereport(ERROR,
					(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
		...
		else if (SxactIsPrepared(writer) && !pc3)
		{
			LWLockRelease(SerializableXactHashLock);
			...
			&& !SxactIsDoomed(nearConflict->sxactOut))
		{
			...
			while (farConflict)
			{
						...
						&& !SxactIsDoomed(farConflict->sxactOut)))
				{
					 ...
					 */
					if (SxactIsPrepared(nearConflict->sxactOut) && !pc3)
					{
						LWLockRelease(SerializableXactHashLock);
						ereport(ERROR,
								(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
