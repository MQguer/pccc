...
public class jTPCCTerminal implements jTPCCConfig, Runnable
{
    ...
    public jTPCCTerminal
      (String terminalName, int terminalWarehouseID, int terminalDistrictID,
       ...
       int deliveryWeight, int stockLevelWeight, int numWarehouses, int limPerMin_Terminal, int i, jTPCC parent,Loadcsv csv) throws SQLException
    {
	...
	if(numTransactions!=-1){
	    ...
	    this.same = new int[numTransactions];  //8  2 重复
	    HashSet<Integer> used = new HashSet<>();  //5
	    for(int j=0;j<numTransactions*0.8;j++) {
	        int seed1;
	        while(true) {
		        if((numTransactions*0.8)%1==0){
		            seed1 = rnd.nextInt(seed*numTransactions, (int) Math.floor(seed*numTransactions+numTransactions*0.8)-1); 
		        }
		        else{
		            seed1 = rnd.nextInt(seed*numTransactions, (int) Math.floor(seed*numTransactions+numTransactions*0.8));
		        }
	...
	for(int i = 0; (i < numTransactions || numTransactions == -1) && !stopRunning; i++)
	{
	    ...
	    if(ooid>=csv.numo) {  //设置规定时间内执行的事务数
	    	break;    
	    }
	    try {
	        // 使当前线程休眠 毫秒
	        Thread.sleep(csv.oorder.daysDifference.get(ooid));
	...
	/*	try
		{
		    term.generatePayment(log, rnd, 0,ooid);
		    term.traceScreen(log);
		    term.execute(log, db);
		    parent.resultAppend(term);
		    term.traceScreen(log);
