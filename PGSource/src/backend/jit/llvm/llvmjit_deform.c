 ...
 */
LLVMValueRef
slot_compile_deform(LLVMJitContext *context, TupleDesc desc, const TupleTableSlotOps *ops, int natts, ModuleType type)
{
	char	   *funcname;
	...
	// mod = llvm_mutable_module(context, "pg");
	mod = llvm_mutable_module_select(context, type);
