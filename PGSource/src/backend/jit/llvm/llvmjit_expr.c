...
llvm_compile_expr(ExprState *state)
{
	...
	ModuleType random_type =  rand() % NUM_TYPES;

	// if(random_type == MODULE_INLINE)
	// {
	// 	random_type += 1;
	// }

	// ModuleType random_type = MODULE_OPT;
	mod = llvm_mutable_module_select(context, random_type);  /*#RAIN NUM_TYPES*/

	b = LLVMCreateBuilder();

	funcname = llvm_expand_funcname(context, "evalexpr");

	printf("[MOD SELECTED] %d %s  [TAG] %d  [FLAGS] %d\n", random_type, funcname, state->tag, state->flags);

	if(random_type == MODULE_DEF)
		return false;
