...
long 
pc3_hash_value(Oid tableid, Oid wid, Oid did, Oid cid, Oid iid) 
{
    long hash			= 	17; 
	long hash_factor 	= 	13; 
    hash = hash * hash_factor + tableid;
    hash = hash * hash_factor + iid;
    hash = hash * hash_factor + cid;
    hash = hash * hash_factor + did;
    hash = hash * hash_factor + wid;
    return hash;
}
...
removeTransactionFromPool(VirtualTransactionId current_vxid)
{
	...
	while(ptr_txn->next)
	{
		if(equalVXID( current_vxid,  ptr_txn->next->vxid))
		{
			...
			TransactionWorkingSet* temp_ptr 		=	ptr_txn->next->next;
			ptr_txn->next->next						=	NULL;
			ptr_txn->next 							= 	temp_ptr;
			break;
		}
		ptr_txn = ptr_txn->next;
	}
...
// build a NewOrderData structure
NewOrderSQLData buildNewOrderData(Oid typeid, Oid tableid, Oid wid, Oid did, Oid cid, Oid iid)
{
	NewOrderSQLData newOrderData = {typeid, tableid, wid, did, cid, iid, pc3_hash_value(tableid, wid, did, cid, iid)};
	return newOrderData;
}

// Find the prediction data in PC3 Cache
PC3HashKey checkPC3Cache(int k_value, NewOrderSQLData *input)
{
	int dataId	=	0;

	PC3HashKey pc3HashKey;
	pc3HashKey.dataId		=		0;

	for(int i=0; i<k_value; i++)
	{
		dataId += input[i].hashValue;

		if(boolPrint)
		{
			printf("input[%d].hashValue = %d \n", i, input[i].hashValue);
		}
			
	}

	int index 			= 	dataId % PCCC_HASH_SIZE;
	int initial_index	=	index;

	if(boolPrint)
	{
		printf("predictionData->pc3HashTable[%d] = (dataId:%d, input:%d, output:%d) \n", index, predictionData->pc3HashTable[index].dataId, predictionData->pc3HashTable[index].inputSize, predictionData->pc3HashTable[index].outputSize);	
		for(int m=0; m<predictionData->pc3HashTable[index].inputSize+predictionData->pc3HashTable[index].outputSize;m++)
		{
			printf("	workingset[%d] = (%d,%d,%d,%d,%d,%d,%d) \n", m, predictionData->pc3HashTable[index].workingset[m].typeid, predictionData->pc3HashTable[index].workingset[m].tableid, predictionData->pc3HashTable[index].workingset[m].wid, predictionData->pc3HashTable[index].workingset[m].did, predictionData->pc3HashTable[index].workingset[m].cid, predictionData->pc3HashTable[index].workingset[m].iid, predictionData->pc3HashTable[index].workingset[m].hashValue);
		}
	}

	while(predictionData->pc3HashTable[index].dataId != 0)
	{
		if(dataId == predictionData->pc3HashTable[index].dataId)
		{
			bool find = true;
			for(int j=0; j<predictionData->pc3HashTable[index].inputSize; j++)
			{
				if(input[j].hashValue != predictionData->pc3HashTable[index].workingset[j].hashValue)
				{
					find	=	false;
					if(boolPrint)
					{
			...
			if(find)
			{
				...
				return predictionData->pc3HashTable[index];
			}

		}

		if(index != PCCC_HASH_SIZE - 1)
		{
			index 	+= 	1;
		}
		else 
	...
	if(boolPrint)
	{
		printf("[ERROR] Cannot Find Prediction Data! \n");
	}
	return pc3HashKey;
}
...
exec_bind_message(StringInfo input_message)
{
	...
	*/
	...
	int				parmValue[10]; 
	int				table_id			=	0;
	CmdType 		query_type 			= 	getSqlStatementType(sql_statement);  // 1select/2update/0commit
	VirtualTransactionId current_vxid 	= 	getVirtualXid();

	if(query_type!=CMD_NOTHING && query_type!=CMD_INSERT)
	{
		typeFlag = true;
	...
	*/
	...
	/************************* #RAIN : PC3 TRANSACTION PREDICTION CACHE ************************/
	NewOrderSQLData orderData = {0,0,0,0,0,0,0};

	if(typeFlag)
	{
		orderData.typeid	=	query_type;
	...
	if(pc3 && typeFlag)
	{
		...
		if(query_type == CMD_UNKNOWN)
		{
			...
			if(boolPrint)
			{
				printf("[Pid:%d, Vxid:%d/%d] Transaction Committed.\n", MyProcPid, current_vxid.backendId, current_vxid.localTransactionId);
			}
		...
		else
		{
			if(index_current_txn >= 0)
			{
				...
				if(new_data_index >= 0)
				{
					...
					if( transactionPool->transactions[index_current_txn]->workingset.inputSize == kValue )
					{
						PC3HashKey txn_workingset 	=	checkPC3Cache(kValue, transactionPool->transactions[index_current_txn]->workingset.workingset);
						
						if(boolPrint)
						{
							printf("[Vxid:%d/%d] InputSize %d, Predicted Item: [ ", transactionPool->transactions[index_current_txn]->workingset.inputSize, current_vxid.backendId, current_vxid.localTransactionId);
							
							for(int k=0; k<transactionPool->transactions[index_current_txn]->workingset.inputSize+transactionPool->transactions[index_current_txn]->workingset.outputSize; k++)
							{
								printf("(%d,", txn_workingset.workingset[k].wid);
								printf("%d,", txn_workingset.workingset[k].did);
								printf("%d,", txn_workingset.workingset[k].cid);
								printf("%d)", txn_workingset.workingset[k].iid);
							}

							printf(" ]\n");
						}
						
						transactionPool->transactions[index_current_txn]->workingset	=	txn_workingset;
						if(txn_workingset.dataId	!=	0)
							result = findConflictedTxn(result, &resultSize, current_vxid, transactionPool->transactions[index_current_txn]);	
					}
